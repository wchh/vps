...
func iat(fp, language string) (string, error) {
	...
	//握手并建立websocket 连接
	conn, _, err := d.Dial(assembleAuthUrl(hostUrl, apiKey, apiSecret), nil)
	if err != nil {
	...
	var frameSize = 12800                //每一帧的音频大小
	var intervel = 20 * time.Millisecond //发送音频间隔
	//开启协程，发送数据
